...
export default async function DashboardOverviewPage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: {
      profile: true,
      mealPlans: { orderBy: { createdAt: "desc" }, take: 1, include: { groceryList: true } },
    },
  });
  ...
  const budgetPct = Math.min(100, Math.round((budgetUsed / profile.weeklyBudgetUsd) * 100));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-ink/60">
              P {profile.proteinTargetG ?? "—"}g · C {profile.carbTargetG ?? "—"}g · F{" "}
              {profile.fatTargetG ?? "—"}g
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">This week's grocery cost</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            {latestPlan ? (
              <>
                ...
                <Link href="/dashboard/meal-plan" className="text-sm font-medium text-forest-700">
                  View plan →
                </Link>
              </>
            ) : (
        ...
        <CardContent className="flex items-center justify-between p-6">
          <div>
            ...
            <p className="text-sm text-white/80">
              Snap a photo of your fridge and we'll build recipes around what's already there.
            </p>
          </div>
